...
class PreNorm(nn.Module):
    ...
    def forward(self, x, a):
        x = self.norm(x)
        return self.fn(torch.cat((x, a), 1))
...
#     def __init__(self, input_size, dim_q, dim_kv, heads=4, dim_head=32):
#         super().__init__()
#         self.scale = dim_head**-0.5
#         self.heads = heads
#         hidden_dim = dim_head * heads
#         self.to_kv = nn.Linear(dim_kv * input_size, hidden_dim * 2 * input_size, bias=False)
#         self.to_q = nn.Linear(dim_q * input_size, hidden_dim * input_size, bias=False)
#         self.to_out = nn.Linear(hidden_dim, dim_q, 1)
